...
```{r ecology1-nex-data, eval=FALSE}
...
```{r corr-convergence-lat, eval=TRUE}
...
```{r corr-convergence-hab, eval=TRUE}
...
```{r ou-data, echo=TRUE, eval=TRUE, message=FALSE, results='hide'}
dat<-read.csv("../data/processed/glmm/Field_comp_dat.csv", sep = ",", header = T, na.strings = c("", "NA"), colClasses = c(rep("factor", 8), rep("numeric",3)))
...
```{r plot-OU-thetas, message=FALSE}
